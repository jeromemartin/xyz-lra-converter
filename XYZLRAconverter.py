...
class MainWindow(QMainWindow, ui.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.actionNew.triggered.connect(self.clearAllContents)
        self.actionImport_Data.triggered.connect(self.importTXT)
        self.actionExport_Data.triggered.connect(self.exportTXT)
        self.actionGenerate_Report_2.triggered.connect(self.generateReport)
        self.LineEditCLR.textChanged.connect(self.clrChanged)
        ...
        self.actionInsertRowXYZ.triggered.connect(functools.partial(self.insertRow, 'XYZ'))
        self.actionDeleteRowXYZ.triggered.connect(functools.partial(self.deleteRow, 'XYZ'))
        ...
        self.actionInsertRowLRA.triggered.connect(functools.partial(self.insertRow, 'LRA'))
        self.actionDeleteRowLRA.triggered.connect(functools.partial(self.deleteRow, 'LRA'))
            ...
            if data is not None:
                ...
                if self.transformation.xyz2lra():  # if convertion succeeded -> update table
                    ...
                    QMessageBox.information(self, 'Error Message',
                                            "Invalid data input. Two data vectors are required for XYZ to LRA conversion.",
                                            QMessageBox.Ok)
        else:
    ...
    def readTableData(self, table):
        ...
        try:
            data = [[float(table.item(row, col).text()) for col in range(self.tableColCount)] for row in
                    range(rowCount)]
        except:  # ValueError or TypeError or AttributeError
            QMessageBox.warning(self, 'Warning', "Invalid table data input: non numeric values or empty cells.",
                                QMessageBox.Ok)  # + str(sys.exc_info()[0])
            return None
        return array(data)
    ...
    def exportTXT(self):
        ...
        if data is not None:
            ...
            QMessageBox.information(self, 'Error Message', "Invalid input. Your data will not be exported.",
                                    QMessageBox.Ok)
    ...
    def readCLR(self):
        try:
            ...
            QMessageBox.information(self, 'Error Message', "Invalid data input. CLR value cannot be updated.",
                                    QMessageBox.Ok)
    ...
    def generateReport(self):
        ...
        try:
            ...
            if data_1 is None or data_1 is None:
                QMessageBox.information(self, 'Error Message', "Invalid input. Some of your data will not be exported.",
                                        QMessageBox.Ok)
